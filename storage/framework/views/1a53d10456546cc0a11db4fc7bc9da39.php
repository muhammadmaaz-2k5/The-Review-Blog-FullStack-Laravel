            ...
            <div class="relative">
                ...
                <div class="relative bg-[#1a1a1a]/80 backdrop-blur-xl border border-white/10 rounded-[1.5rem] p-8 md:p-10 shadow-2xl">
                    ...
                    <?php if(session('success')): ?>
                        <div class="mb-6 p-4 bg-green-500/10 border border-green-500/30 text-green-400 rounded-xl flex items-start gap-3">
                    ...
                    <form action="<?php echo e(route('tips.store')); ?>" method="POST" class="space-y-6">
                        <?php echo csrf_field(); ?>
                        
                        <div class="space-y-2">
                            <label for="subject" class="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">
                                Subject <span class="text-accent">*</span>
                            </label>
                            <input type="text" id="subject" name="subject" required
                                   value="<?php echo e(old('subject')); ?>"
                                   class="w-full px-5 py-4 bg-black/40 border border-white/10 rounded-xl focus:ring-2 focus:ring-accent focus:border-transparent text-white font-medium transition-all placeholder-gray-600 hover:bg-black/60"
                                   placeholder="E.g. New smartphone leaks...">
                        </div>

                        <div class="space-y-2">
                            <label for="content" class="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">
                                The Story <span class="text-accent">*</span>
                            </label>
                            <textarea id="content" name="content" rows="6" required
                                      class="w-full px-5 py-4 bg-black/40 border border-white/10 rounded-xl focus:ring-2 focus:ring-accent focus:border-transparent text-white font-medium transition-all placeholder-gray-600 hover:bg-black/60 resize-none"
                                      placeholder="Tell us everything... (Min 10 chars)"><?php echo e(old('content')); ?></textarea>
                            <p class="text-[10px] text-gray-500 text-right uppercase tracking-wide font-bold">
                                Links: Imgur / YouTube supported
                            </p>
                        </div>

                        <div class="space-y-2">
                            <label for="captcha_answer" class="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">
                                Security Check <span class="text-accent">*</span>
                            </label>
                            <div class="relative">
                                <div class="absolute left-5 top-1/2 -translate-y-1/2 text-accent font-bold font-mono text-lg z-20">
                                    <?php echo e($captchaQuestion ?? '42 = ?'); ?>

                                </div>
                                <input type="number" id="captcha_answer" name="captcha_answer" required
                                       class="w-full pl-32 pr-5 py-4 bg-black/40 border border-white/10 rounded-xl focus:ring-2 focus:ring-accent focus:border-transparent text-white font-medium transition-all placeholder-gray-600 hover:bg-black/60 relative z-10"
                                       placeholder="Answer">
                            </div>
                        </div>

                        <button type="submit" class="w-full group relative px-8 py-4 bg-accent hover:bg-red-700 text-white font-bold rounded-xl transition-all shadow-lg hover:shadow-accent/40 overflow-hidden">
                            <div class="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-20"></div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\asdfq\Desktop\New folder (2)\resources\views/tips/create.blade.php ENDPATH**/ ?>