...
<div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-16">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        @forelse($series as $ser)
            <a href="{{ route('series.show', $ser->slug) }}" class="group flex flex-col bg-white dark:!bg-[#1a1a1a] rounded-2xl overflow-hidden border border-gray-100 dark:!border-white/5 shadow-lg hover:shadow-2xl hover:shadow-accent/10 transition-all duration-300 transform hover:-translate-y-1 h-full">
                <!-- Series Image -->
                <div class="relative aspect-[3/4] overflow-hidden">
                    @if($ser->featured_image)
                        ...
                        @endphp
                        <img src="{{ $imageUrl }}" 
                             alt="{{ $ser->featured_image_alt ?: $ser->title }}" 
                ...
                <div class="p-6 flex-1 flex flex-col relative bg-white dark:!bg-[#1a1a1a]">
                    ...
                    <div class="absolute -top-5 right-4 bg-accent text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg">
                        {{ number_format($ser->articles_count ?? 0) }} Parts
                    </div>
                            ...
                            @if($ser->author)
                                <span class="flex items-center gap-1.5">
                                    <div class="w-5 h-5 rounded-full bg-gray-200 dark:!bg-gray-700 overflow-hidden">
                                        @if($ser->author->avatar)
                                            <img src="{{ $ser->author->avatar }}" class="w-full h-full object-cover">
                                        @else
                                            <div class="w-full h-full flex items-center justify-center text-[8px] text-gray-500">{{ substr($ser->author->name, 0, 1) }}</div>
                                        @endif
                                    </div>
                                    {{ $ser->author->name }}
                                </span>
