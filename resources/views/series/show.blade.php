    ...
    <div class="absolute inset-0 bg-gray-900">
        ...
        @if($series->featured_image)
            <div class="absolute inset-0 bg-cover bg-center transition-transform duration-[20s] hover:scale-105" 
                 style="background-image: url('{{ $series->featured_image_url }}');"></div>
        @else
                ...
                <div class="flex items-center gap-4 mb-4">
                    ...
                    @if($seriesStats['avg_reading_time'] > 0)
                        <span class="flex items-center gap-1 text-gray-300 text-sm font-medium">
                            ...
                            {{ $seriesStats['avg_reading_time'] }} min avg
                        </span>
                    @endif
                    <span class="flex items-center gap-1 text-gray-300 text-sm font-medium">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                        {{ number_format($seriesStats['total_views']) }} Views
                    </span>
                </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    @if($series->author)
                        <div class="flex items-center gap-3 px-6 py-3 bg-white/10 backdrop-blur-md rounded-full border border-white/10">
                            ...
                            <span class="text-white font-bold">{{ $series->author->name }}</span>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="lg:col-span-8" id="episodes">
            <div class="flex items-center justify-between mb-8">
                <h2 class="text-2xl font-bold text-gray-900 dark:!text-white uppercase tracking-tight flex items-center gap-3">
                    ...
                    Episodes <span class="text-gray-500 text-lg font-normal">({{ $series->articles->count() }})</span>
                </h2>
            </div>
            ...
            @if($series->articles->count() > 0)
                <div class="space-y-6">
                    @foreach($series->articles as $article)
                        <a href="{{ route('articles.show', $article->slug) }}" class="group flex flex-col md:flex-row gap-6 p-4 rounded-2xl hover:bg-gray-50 dark:!hover:bg-white/5 transition-all border border-transparent hover:border-gray-100 dark:!hover:border-white/5">
                            <!-- Episode Thumbnail -->
                            ...
                            <div class="flex-1 py-2">
                                <div class="flex items-center justify-between mb-2">
                                    ...
                                    @if($article->reading_time)
                                        <span class="text-xs font-medium text-gray-400 shrink-0">{{ $article->reading_time }}m</span>
                                    @endif
                                </div>
                                ...
                                <div class="flex items-center gap-4 text-xs font-medium text-gray-400 uppercase tracking-wide">
                                    ...
                                    @endif
                                    @if($article->category)
                                        <span class="text-accent">{{ $article->category->name }}</span>
                                    @endif
                                </div>
                            </div>
                        </a>
        ...
        <div class="lg:col-span-4 space-y-8">
            ...
            <div class="bg-gray-50 dark:!bg-[#1a1a1a] rounded-2xl p-6 border border-gray-100 dark:!border-white/5">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="bg-white dark:!bg-black/20 p-4 rounded-xl">
                        <div class="text-2xl font-black text-gray-900 dark:!text-white">{{ number_format($seriesStats['total_articles']) }}</div>
                        <div class="text-xs text-gray-500 uppercase font-bold tracking-wide">Episodes</div>
                    </div>
                    <div class="bg-white dark:!bg-black/20 p-4 rounded-xl">
                        <div class="text-2xl font-black text-gray-900 dark:!text-white">{{ number_format($seriesStats['total_views']) }}</div>
            ...
            @if($popularInSeries->count() > 0)
                <div class="bg-gray-50 dark:!bg-[#1a1a1a] rounded-2xl p-6 border border-gray-100 dark:!border-white/5">
                    ...
                    <div class="space-y-4">
                        @foreach($popularInSeries as $popular)
                            <a href="{{ route('articles.show', $popular->slug) }}" class="flex gap-4 group">
                                <div class="w-20 h-28 flex-shrink-0 rounded-lg overflow-hidden bg-gray-200 relative">
                                    @if($popular->featured_image)
                                        <img src="{{ $popular->featured_image_url }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                                    @endif
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        @foreach($relatedSeries as $related)
                            <a href="{{ route('series.show', $related->slug) }}" class="group relative aspect-[2/1] rounded-xl overflow-hidden shadow-lg">
                                @if($related->featured_image)
                                    @php
                                        $relImg = str_starts_with($related->featured_image, 'http') ? $related->featured_image : asset('storage/' . $related->featured_image);
                                    @endphp
                                    <img src="{{ $relImg }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                                @else
                                ...
                                <div class="absolute bottom-0 left-0 p-4">
                                    ...
                                    <span class="text-xs text-gray-300">{{ $related->articles_count ?? 0 }} Episodes</span>
                                </div>
                            </a>
                        @endforeach
                    </div>
