    ...
    <div class="p-6 md:p-8">
        ...
        <div class="space-y-4">
            @foreach($faqs as $index => $faq)
                <div class="faq-item border border-gray-200 dark:!border-border-secondary rounded-lg overflow-hidden" x-data="{ open: {{ $index === 0 ? 'true' : 'false' }} }">
                    <button @click="open = !open" class="w-full px-5 py-4 text-left flex justify-between items-center bg-gray-50 dark:!bg-bg-card-hover hover:bg-gray-100 dark:hover:!bg-gray-800 transition-colors focus:outline-none">
                        <span class="font-semibold text-gray-900 dark:!text-white pr-4" style="font-weight:600;">{{ $faq['question'] }}</span>
