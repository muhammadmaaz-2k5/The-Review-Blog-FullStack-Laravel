...
@if(isset($mainHero) && $mainHero->featured_image)
<link rel="preload" as="image" href="{{ $mainHero->featured_image_url }}" fetchpriority="high">
@endif
@foreach($subHeroes as $sub)
    @if($sub->featured_image)
<link rel="preload" as="image" href="{{ $sub->featured_image_url }}" fetchpriority="high">
    @endif
@endforeach
...
<div class="homepage-container">
    ...
    <div class="hero-wrapper">
        <!-- Main Hero (Left) -->
        <a href="{{ route('articles.show', $mainHero->slug) }}" class="hero-main">
            @if($mainHero->featured_image)
                <img src="{{ $mainHero->featured_image_url }}" alt="{{ $mainHero->title }}" class="hero-bg" fetchpriority="high" loading="eager">
            @else
                <div class="hero-bg placeholder-bg"></div>
            @endif
            <div class="hero-content">
                <span class="hero-tag">{{ $mainHero->category->name ?? 'Featured' }}</span>
                <h1 class="hero-title">
                    {{ $mainHero->title }}
                </h1>
                <div class="hero-meta">
                    By {{ $mainHero->author->name ?? 'Nazaara Team' }} • {{ $mainHero->published_at ? $mainHero->published_at->format('F d, Y') : 'Just Now' }}
                </div>
            </div>
        </a>
        ...
        <div class="hero-sub">
            @foreach($subHeroes as $sub)
            <a href="{{ route('articles.show', $sub->slug) }}" class="hero-sub-item">
                @if($sub->featured_image)
                    <img src="{{ $sub->featured_image_url }}" alt="{{ $sub->title }}" class="hero-bg" fetchpriority="high" loading="eager">
                @else
                    <div class="hero-bg placeholder-bg"></div>
                @endif
                <div class="hero-content">
                    <span class="hero-tag">{{ $sub->category->name ?? 'Trending' }}</span>
                    <h3 class="hero-title">
                        {{ Str::limit($sub->title, 50) }}
                    </h3>
                    <div class="hero-meta">
                        {{ $sub->reading_time ?? 5 }} min read
                    </div>
                </div>
            </a>
            @endforeach
    ...
    <div class="entertainment-grid" id="home-articles-container">
        @forelse($articles as $article)
            {{-- Skip if already shown in hero --}}
            @if(isset($mainHero) && $article->id === $mainHero->id) @continue @endif
            @if($subHeroes->contains('id', $article->id)) @continue @endif
    ...
    <div id="home-load-more-trigger" class="py-8 text-center" data-page="2" data-url="{{ route('articles.load-more') }}" data-has-more="{{ $articles->hasMorePages() ? 'true' : 'false' }}">
    ...
    @if(isset($top10Category) && $top10Category && isset($top10Articles) && $top10Articles->count() > 0)
    <div class="homepage-container mt-16">
        <div class="section-header mb-8">
            <h2 class="section-title" style="color: {{ $top10Category->color ?? '#E50914' }};">
                {{ $top10Category->name }}
            </h2>
            <a href="{{ route('categories.show', $top10Category->slug) }}" class="text-accent hover:text-accent-light font-bold text-sm uppercase tracking-widest flex items-center gap-2 transition-all">
                View All
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
            @foreach($top10Articles as $index => $article)
                ...
                <div class="aspect-[16/9] overflow-hidden">
                    ...
                    @else
                        <div class="w-full h-full bg-gradient-to-br from-accent to-purple-600 flex items-center justify-center text-white text-2xl font-black">
                            {{ strtoupper(substr($article->title, 0, 2)) }}
                        </div>
                    @endif
                </div>
                ...
                <div class="p-4">
                    ...
                    <div class="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                        <div class="flex items-center gap-1">
                            ...
                            <span class="font-bold">{{ number_format($article->views) }}</span>
                        </div>
                        <div class="flex items-center gap-1">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @if(isset($instagramStarCategory) && $instagramStarCategory && $instagramStarArticles && $instagramStarArticles->count() > 0)
    <div class="mt-16">
        <div class="section-header mb-8">
            <h2 class="section-title" style="color: {{ $instagramStarCategory->color ?? '#E50914' }};">
                {{ $instagramStarCategory->name }}
            </h2>
            <a href="{{ route('categories.show', $instagramStarCategory->slug) }}" class="text-accent hover:text-accent-light font-bold text-sm uppercase tracking-widest flex items-center gap-2 transition-all">
                View All
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
            </a>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            @foreach($instagramStarArticles as $article)
            <div class="group bg-white dark:!bg-bg-card rounded-2xl border border-gray-200 dark:!border-border-secondary overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2">
                <a href="{{ route('articles.show', $article->slug) }}" class="block relative aspect-[16/9] overflow-hidden">
                    @if($article->featured_image)
                        <img src="{{ $article->featured_image_url }}" alt="{{ $article->title }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" loading="lazy">
                    @else
                        <div class="w-full h-full bg-gradient-to-br from-purple-600 to-pink-600 flex items-center justify-center text-white text-3xl font-black">{{ strtoupper(substr($article->title, 0, 2)) }}</div>
                    @endif
                ...
                <div class="p-5">
                    ...
                    <p class="text-sm text-gray-600 dark:!text-text-secondary mb-4 line-clamp-2" style="font-family: 'Poppins', sans-serif;">{{ Str::limit($article->excerpt ?: $article->description, 80) }}</p>
                    <div class="flex items-center justify-between mt-4">
                        ...
                        <a href="{{ route('articles.show', $article->slug) }}" class="text-[10px] font-black text-accent uppercase tracking-widest flex items-center gap-1 group/link">
    ...
    {{-- Download Process Overlay (Only for NazaaraBox redirects) --}}
        ...
        <div id="download-process-wrapper" class="fixed inset-0 z-[9999] flex items-center justify-center bg-black/95 backdrop-blur-md p-4 transition-all duration-500">
            <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-md w-full p-8 text-center border border-gray-200 dark:border-gray-700 relative overflow-hidden">
                ...
                <div id="step-4" class="hidden transition-opacity duration-300">
                    ...
                    <a href="{{ base64_decode(request('destination')) }}" target="_blank"
    ...
    @if(isset($hotCelebritiesCategory) && $hotCelebritiesCategory && $hotCelebritiesArticles && $hotCelebritiesArticles->count() > 0)
    <div class="mt-16">
        <div class="section-header mb-8">
            <h2 class="section-title" style="color: {{ $hotCelebritiesCategory->color ?? '#E50914' }};">
                {{ $hotCelebritiesCategory->name }}
            </h2>
            <a href="{{ route('categories.show', $hotCelebritiesCategory->slug) }}" class="text-accent hover:text-accent-light font-bold text-sm uppercase tracking-widest flex items-center gap-2 transition-all">
                View All
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
            </a>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            @foreach($hotCelebritiesArticles as $article)
            <div class="group bg-white dark:!bg-bg-card rounded-2xl border border-gray-200 dark:!border-border-secondary overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2">
                <a href="{{ route('articles.show', $article->slug) }}" class="block relative aspect-[16/9] overflow-hidden">
                    @if($article->featured_image)
                        <img src="{{ $article->featured_image_url }}" alt="{{ $article->title }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" loading="lazy">
                    @else
                        <div class="w-full h-full bg-gradient-to-br from-pink-600 to-red-600 flex items-center justify-center text-white text-3xl font-black">{{ strtoupper(substr($article->title, 0, 2)) }}</div>
                    @endif
                    <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-60 group-hover:opacity-40 transition-opacity"></div>
