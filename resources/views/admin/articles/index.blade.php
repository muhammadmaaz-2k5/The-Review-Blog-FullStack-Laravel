...
@section('title', 'Admin - Article Management')
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-6 flex items-center justify-between">
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('admin.articles.create') }}" class="px-4 py-2 bg-accent hover:bg-accent-light text-white rounded-lg transition-colors" style="font-family: 'Poppins', sans-serif; font-weight: 600;">
                Add New Article
            </a>
        </div>
    </div>
    ...
    <div class="mb-6 bg-white dark:!bg-bg-card rounded-lg border border-gray-200 dark:!border-border-secondary p-4">
        <form method="GET" action="{{ route('admin.articles.index') }}" class="flex flex-wrap gap-4 items-end">
            ...
            <div>
                ...
                <a href="{{ route('admin.articles.index') }}" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-900 rounded-lg transition-colors dark:!bg-bg-card dark:!text-white dark:!hover:bg-bg-card-hover ml-2" style="font-family: 'Poppins', sans-serif; font-weight: 600;">
                    Clear
                </a>
            </div>
        </form>
                        ...
                        <tr class="hover:bg-gray-50 dark:!hover:bg-bg-card-hover transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-2">
                                    <button onclick="postToFacebook('{{ $article->id }}', this)" 
                                            class="text-blue-600 hover:text-blue-900 dark:!text-blue-400 dark:!hover:text-blue-300 {{ ($article->status !== 'published' || !config('services.facebook.enabled', false)) ? 'opacity-50 cursor-not-allowed' : '' }}" 
                                            title="{{ $article->status !== 'published' ? 'Only published articles can be posted' : (config('services.facebook.enabled', false) ? 'Post to Facebook' : 'Facebook integration not enabled') }}"
                                            id="fb-btn-{{ $article->id }}"
                                            {{ ($article->status !== 'published' || !config('services.facebook.enabled', false)) ? 'disabled' : '' }}>
                                    ...
                                    <a href="{{ route('admin.articles.edit', $article) }}" class="text-accent hover:text-accent-light" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-gray-600 dark:!text-text-secondary" style="font-family: 'Poppins', sans-serif; font-weight: 400;">
                                No articles found. <a href="{{ route('admin.articles.create') }}" class="text-accent hover:underline">Create your first article</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
