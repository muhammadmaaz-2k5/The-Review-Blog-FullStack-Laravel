            ...
            @forelse($files as $file)
                <div class="group relative rounded-lg border border-gray-200 dark:!border-border-secondary overflow-hidden bg-gray-50 dark:!bg-bg-card-hover transition-all hover:shadow-lg">
                    <div class="aspect-square relative flex items-center justify-center overflow-hidden">
                        @if(in_array(strtolower($file['extension']), ['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg']))
                        ...
                        <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                            <a href="{{ $file['url'] }}" target="_blank" class="p-2 bg-white rounded-full text-gray-900 hover:bg-gray-100" title="View">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                </svg>
                            </a>
                            <button onclick="copyToClipboard('{{ $file['url'] }}')" class="p-2 bg-white rounded-full text-purple-600 hover:bg-gray-100" title="Copy URL">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
                                </svg>
                            </button>
                            <form action="{{ route('admin.media.destroy') }}" method="POST" onsubmit="return confirm('Delete this file?');">
                                @csrf
                                @method('DELETE')
                    ...
                    <div class="p-2">
                        <p class="text-[10px] text-gray-500 truncate" title="{{ $file['name'] }}">{{ $file['name'] }}</p>
                        <p class="text-[10px] font-bold text-accent">{{ strtoupper($file['extension']) }} • {{ number_format($file['size'] / 1024, 1) }} KB</p>
                        <p class="text-[9px] text-gray-400 capitalize">{{ str_replace('articles/', '', $file['directory']) }}</p>
                    </div>
                </div>
            @empty
