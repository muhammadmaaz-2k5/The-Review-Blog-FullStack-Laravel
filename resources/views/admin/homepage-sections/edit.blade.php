    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600 dark:!text-text-secondary mt-1" style="font-family: 'Poppins', sans-serif; font-weight: 400;">
                Update section: {{ $homepageSection->name }}
            </p>
        </div>
    </div>
        ...
        <form action="{{ route('admin.homepage-sections.update', $homepageSection) }}" method="POST">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $homepageSection->name) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="slug" value="{{ old('slug', $homepageSection->slug) }}" required
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-accent focus:border-transparent dark:!bg-bg-card-hover dark:!border-border-primary dark:!text-white"
                           placeholder="e.g., biographies">
                ...
                <div>
                    ...
                    <input type="text" name="section_title" value="{{ old('section_title', $homepageSection->section_title) }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-accent focus:border-transparent dark:!bg-bg-card-hover dark:!border-border-primary dark:!text-white"
                ...
                <div>
                    ...
                    <input type="number" name="display_order" value="{{ old('display_order', $homepageSection->display_order) }}" min="0"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-accent focus:border-transparent dark:!bg-bg-card-hover dark:!border-border-primary dark:!text-white"
                           placeholder="0">
                ...
                <div>
                    ...
                    <input type="number" name="articles_per_section" value="{{ old('articles_per_section', $homepageSection->articles_per_section) }}" min="1" max="20"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-accent focus:border-transparent dark:!bg-bg-card-hover dark:!border-border-primary dark:!text-white"
                           placeholder="4">
                        ...
                        @foreach($categories as $category)
                            <label class="flex items-center gap-2 cursor-pointer p-2 rounded hover:bg-gray-50 dark:!hover:bg-bg-card-hover transition-colors">
                                <input type="checkbox" name="category_ids[]" value="{{ $category->id }}"
                                       {{ in_array($category->id, old('category_ids', $homepageSection->category_ids ?? [])) ? 'checked' : '' }}
                ...
                <div>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $homepageSection->is_active) ? 'checked' : '' }}
                               class="rounded border-gray-300 text-accent focus:ring-accent">
