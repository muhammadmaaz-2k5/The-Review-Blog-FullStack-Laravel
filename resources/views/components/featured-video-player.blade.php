@if(isset($randomFeaturedVideo) && $randomFeaturedVideo)
    ...
    <div id="featured-video-container"
        ...
        <div class="bg-accent/90 backdrop-blur-sm text-white px-3 py-1 flex items-center justify-between text-xs font-bold">
            ...
            <button onclick="closeFeaturedVideo()" class="hover:bg-accent-light p-1 rounded-full transition-colors">
    ...
    <script>
        function closeFeaturedVideo() {
            ...
            if (container) {
                ...
                sessionStorage.setItem('featuredVideoClosed', 'true');
            }
        }

        document.addEventListener('DOMContentLoaded', function() {
            if (sessionStorage.getItem('featuredVideoClosed') === 'true') {
                const container = document.getElementById('featured-video-container');
