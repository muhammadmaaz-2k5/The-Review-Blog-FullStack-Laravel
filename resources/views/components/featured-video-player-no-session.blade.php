@if(isset($randomFeaturedVideo) && $randomFeaturedVideo)
    ...
    <script>
        function closeFeaturedVideoNoSession() {
            const container = document.getElementById('featured-video-container');
            if (container) {
                container.style.transform = 'translateX(-120%)';
                setTimeout(() => container.remove(), 300);
                // Don't use sessionStorage - just close it
            }
        }

        // Log that this component is loaded (for debugging)
        console.log('🎬 Featured Video Component (No Session) Loaded!');
    ...
    <style>
        ...
        @media (max-width: 640px) {
            ...
            #featured-video-container.is-short {
                width: 140px; /* Even smaller for portrait video */
            }
            #featured-video-container.is-standard {
...
@else
    <script>
        console.log('❌ Featured Video: No video available or variable not set');
    </script>
@endif